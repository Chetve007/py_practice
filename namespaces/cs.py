...
class SC:
    scv = 'super class variable: self.scv'
    ...
    def __init__(self):
        ...
        self.__psiv = 'private superclassinstance variable: no access'

    def sm(self):
        return 'superclass method: self.sm()'

    def __spm(self):
        return 'superclass private method: self.__spm() - no access'


class C(SC):
    cv = 'class variable: self.cv (but use C.cv for assignment)'
    __pcv = 'class private variable: self__pcv (but use C.__pcv for assignment)'
